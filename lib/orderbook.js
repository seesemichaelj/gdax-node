...
class Orderbook {
  ...
  aggregateState(numPricesPerType) {
    let bidPrices = [];
    let bidVolumes = [];
    let askPrices = [];
    let askVolumes = [];

    let it = this._bids.iterator();
    let bid;
    while ((bid = it.prev()) !== null && bidPrices.length < numPricesPerType) {
      // do stuff with item
      bid.orders.forEach(order => {
        const price = parseFloat(order.price);
        const size = parseFloat(order.size);
        let pos = bidPrices.indexOf(price);
        if (pos >= 0) {
          bidVolumes[pos] += size;
        } else {
          bidPrices.push(price);
          bidVolumes.push(size);
        }
      });
    }

    it = this._asks.iterator();
    let ask;
    while ((ask = it.next()) !== null && askPrices.length < numPricesPerType) {
      // do stuff with item
      ask.orders.forEach(order => {
        const price = parseFloat(order.price);
        const size = parseFloat(order.size);
        let pos = askPrices.indexOf(price);
        if (pos >= 0) {
          askVolumes[pos] += size;
        } else {
          askPrices.push(price);
          askVolumes.push(size);
        }
      });
    }
